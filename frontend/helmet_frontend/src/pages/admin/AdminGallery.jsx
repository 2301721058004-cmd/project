...
export function AdminGallery() {
  ...
  const fetchDetections = async () => {
    try {
      setLoading(true);
      const response = await api.detection.getEvents();
      const detectionsData = Array.isArray(response.events) ? response.events : [];
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-gray-800 mb-2">System Detection Gallery</h1>
        <p className="text-gray-600">View all detections from all supervisors with images and analysis</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-sm text-gray-600 mb-1">System Violations</p>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div>
            ...
            >
              ...
              <option value="violations">System Violations ({stats.violations})</option>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-lg border ${viewMode === 'grid' ? 'bg-orange-500 text-white border-orange-500' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
              >
                🔲 Grid
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`px-4 py-2 rounded-lg border ${viewMode === 'list' ? 'bg-orange-500 text-white border-orange-500' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
              >
                ≡ List
              </button>
            </div>
      ...
      {filteredDetections.length === 0 ? (
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">Try adjusting your filters or wait for supervisors to upload detection files</p>
          </div>
        </Card>
      ) : (
        <>
                ...
                >
                  ...
                  <div className={`p-4 ${detection.has_violation ? 'bg-red-50' : 'bg-green-50'}`}>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-gray-500">
                        Detected by: <span className="font-semibold">{detection.uploaded_by_name || 'System'}</span>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-100 rounded-lg border border-gray-300 overflow-hidden">
                  {selectedDetection.file_type === 'video' ? (
                    ...
                    <img
                      ...
                      onError={(e) => {
                        e.target.style.display = 'none';
                        e.target.parentElement.innerHTML = '<div style="padding:2rem; text-align:center; color:#9ca3af;"><p>Failed to load image</p></div>';
                      }}
                    />
                  ) : (
                    <div className="p-12 text-center text-gray-400">No image available</div>
                  )}
                </div>
              </div>
